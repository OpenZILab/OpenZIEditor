...
function updateTrackList() {
    ...
    if (streamSelector && trackSelector) {
        const stream = webRtcPlayerObj.availableVideoStreams.get(streamSelector.value);
...
function setupWebRtcPlayer(htmlElement, config) {
    ...
    webRtcPlayerObj.onNewVideoTrack = function (streams) {
        console.log(`webRtcPlayerObj.onNewVideoTrack app.js line:822`)
...
function resizePlayerStyleToFillWindow(playerElement) {
    ...
    let streamingVideo = document.getElementById('streamingVideo');
    ...
    // TODO: 从填充切换回等比缩放后，将画面分辨率重设为等比的分辨率；从等比切换回填充后，将画面分辨率重设为填充分辨率
    let checkBox = document.getElementById('select-and-uniform-scale')
    // if (checkBox) {
    ...
    if (checkBox && checkBox.checked) {
        // 设置video样式为等比自适应缩放
        if (streamingVideo) {
            streamingVideo.style = "position: absolute; width: 100%; height: 100%; left: 0; top: 0;";
        }
        ...
        // 设置video样式为填充
        if (streamingVideo) {
            streamingVideo.style = "position: fixed; width: 100%; height: 100%; left: 0; top: 0; object-fit: fill;";
        }
        styleWidth = window.innerWidth;
        styleHeight = window.innerHeight;
        styleTop = 0;
...
function resizePlayerStyle(event) {
    ...
    if (checkBox !== null) {
        ...
        // 因为html中关闭了配置面板，所以会执行到此处，UE默认是FillWindow的
        resizePlayerStyleToFillWindow(playerElement);
    }
...
function setupMouseAndFreezeFrame(playerElement) {
    ...
    let checkBox = document.getElementById('select-and-uniform-scale')
    if ((checkBox && !(checkBox.checked)) || !checkBox) {
        setupNormalizeAndQuantize(true)
    } else {
        setupNormalizeAndQuantize(false);
    }
    resizeFreezeFrameOverlay();
}
...
function updateVideoStreamSize() {
    ...
    if ((checkBox && !(checkBox.checked)) || !checkBox) {
        ...
        if (now - lastTimeResized > 1000) {
            ...
            let descriptor = {
                ConsoleCommand: 'r.setres ' + playerElement.clientWidth + 'x' + playerElement.clientHeight + 'w'
            };
...
function emitDescriptor(messageType, descriptor) {
    ...
    console.log(`descriptorAsString ${descriptorAsString}`)
...
function setupNormalizeAndQuantize(bFillMode) {
    let playerElement = document.getElementById('player');
    let videoElement = undefined
    if (bFillMode) {

    } else {
        videoElement = playerElement.getElementsByTagName("video");
    }
    ...
    if (playerElement) {
        ...
        if (bFillMode) {
            videoAspectRatio = playerElement.clientHeight / playerElement.clientWidth
        } else {
            if (videoElement.length > 0) {
                videoAspectRatio = videoElement[0].videoHeight / videoElement[0].videoWidth;
            }
        }
