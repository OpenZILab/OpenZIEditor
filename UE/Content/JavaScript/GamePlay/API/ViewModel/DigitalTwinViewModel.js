...
exports.DigitalTwinViewModel = void 0;
...
const DigitalTwinSystem_1 = require("../../../System/Project/DigitalTwin/DigitalTwinSystem");
const SceneNodeUtil_1 = require("../../../System/Project/Scene/SceneNodeUtil");
const DigitalTwinModel_1 = require("../Model/DigitalTwinModel");
const DigitalTwinNode_1 = require("../../../System/Project/Scene/SceneType/DigitalTwinNode");
...
class DigitalTwinViewModel extends BaseViewModel_1.BaseViewModel {
    bLoadDataSmithing;
    constructor() {
        super();
        this.BaseModel = new DigitalTwinModel_1.DigitalTwinModel();
        //this._OBJClass = makeUClass(PointView)
        this.Type = "DigitalTwin";
        this.Birthplace = "Scene";
        this.bLoadDataSmithing = false;
        MessageManager_1.MessageCenter.Add(this, this.OnDataSmithStartLoad, NotificationLists_1.NotificationLists.DATASMITH.ON_DATASMITH_START_LOAD);
        MessageManager_1.MessageCenter.Add(this, this.OnDataSmithLoadEnd, NotificationLists_1.NotificationLists.DATASMITH.ON_DATASMITH_LOAD_END);
    }
    ...
    ExecuteAdd(jsonData) {
        ...
        let curActor = (0, QEngine_1.GetSystem)(DigitalTwinSystem_1.DigitalTwinSystem).LoadDigitalTwinFormStr(_data.data.digitalTwinContent, (0, puerts_1.$unref)(EngineLocation), null, null, id);
        if (curActor !== null) {
    ...
    ToggleTimeline(jsonData) {
        ...
        if (this.bLoadDataSmithing == true) {
            ...
            if (this.OBJMaps.has(jsonData.data.id)) {
                let Node = SceneNodeUtil_1.NodeHelper.FindNodeByActor(this.OBJMaps.get(jsonData.data.id));
                if (Node instanceof DigitalTwinNode_1.DigitalTwinNode) {
    ...
    ExecuteShow(jsondata) {
        ...
        for (let entry of ids) {
            let Node = SceneNodeUtil_1.NodeHelper.FindNodeByActor(this.OBJMaps.get(entry));
            if (!Node)
                continue;
            let AllChildNodes = [];
    ...
    ExecuteAllShow(jsondata) {
        for (let value of this.OBJMaps.keys()) {
            let Node = SceneNodeUtil_1.NodeHelper.FindNodeByActor(this.OBJMaps.get(value));
            let AllChildNodes = [];
            SceneNodeUtil_1.NodeHelper.GetNodeAllChilds(Node, AllChildNodes);
            AllChildNodes.forEach(item => {
                item.SetHiddenNode(false);
    ...
    ExecuteClear(jsondata) {
        for (let value of this.OBJMaps.values()) {
            if (value !== null) {
                ...
                let Node = SceneNodeUtil_1.NodeHelper.FindNodeByActor(value);
                if (!Node)
                    continue;
                let ParentNode = Node.GetParent();
                let AllChildNodes = [];
    ...
    GetTwinAPIList(jsonData) {
        ...
        //     jsonData.data.result = result
        // }
        let Str = (0, QEngine_1.GetSystem)(DigitalTwinSystem_1.DigitalTwinSystem).GetDigitalTwinStr(jsonData.data.type);
        jsonData.data.result = Str;
        let message = (0, IAPIMessageHandle_1.PackCallBacKMessage)(jsonData, jsonData.data);
        WebSocketServer_1.WebSocketServer.GetInstance().OnSendWebMessage(message);
    }
}
exports.DigitalTwinViewModel = DigitalTwinViewModel;
//# sourceMappingURL=DigitalTwinViewModel.js.map