...
const PropertyCopy_1 = require("../../Core/PropertyCopy");
...
class BaseModel {
    ...
    ProcessData(jsonData, defaultData, defaultDataRange) {
        let curData = PropertyCopy_1.PropertyCopy.SimpleCopy(jsonData);
        let ruleData = PropertyCopy_1.PropertyCopy.SimpleCopy(defaultData);
        let ruleDateRange = PropertyCopy_1.PropertyCopy.SimpleCopy(defaultDataRange);
        if (curData != null) {
                    ...
                    if (curData[key] instanceof Array) {
                        if (typeof ruleData[key][0] === "object" && typeof curData[key][0] === "string") {
                            ...
                            for (var i = 0; i < curData[key].length; i++) {
                                let curStr = curData[key][i];
                                let outStr = curStr.split(",");
                                let index = 0;
                                let OneObj = {};
                                if (SingleNum == 2)
                                    OneObj = { X: Number, Y: Number };
                            ...
                            for (let i = 0; i < curData[key].length; i++) {
                                if (ruleData[key].length < i + 1) {
                                    ruleData[key][i] = PropertyCopy_1.PropertyCopy.DeepCopy(ruleData[key][0]);
                                }
    ...
    GenDefaultRange(typeName, funcName) {
        let OutRangeData = {};
        ...
        actionData_Dev.jsondata.forEach((item) => {
            if (item.range) {
                ...
                if (item.class == "Integer") {
                    ...
                    let valueMin = item.range.min;
                    valueMin = {}[valueMin.X, valueMin.Y] = this.ConvertToObject(item.range.min);
                    RangeValue.min = this.ArraytoVector(valueMin);
                    let valueMax = item.range.max;
                    valueMax = {}[valueMax.X, valueMax.Y] = this.ConvertToObject(item.range.max);
                    RangeValue.max = this.ArraytoVector(valueMax);
                }
                else if (item.class.indexOf("Vector3") != -1) {
                    let valueMin = item.range.min;
                    valueMin = {}[valueMin.X, valueMin.Y, valueMin.Z] = this.ConvertToObject(item.range.min);
                    this.ArraytoVector(valueMin);
                    RangeValue.min = valueMin;
                    let valueMax = item.range.max;
                    valueMax = {}[valueMax.X, valueMax.Y, valueMax.Z] = this.ConvertToObject(item.range.max);
                    RangeValue.max = this.ArraytoVector(valueMax);
                }
                else if (item.class.indexOf("Vector4") != -1) {
                    let valueMin = item.range.min;
                    valueMin = {}[valueMin.X, valueMin.Y, valueMin.Z, valueMin.W] = this.ConvertToObject(item.range.min);
                    RangeValue.min = this.ArraytoVector(valueMin);
                    let valueMax = item.range.max;
                    valueMax = {}[valueMax.X, valueMax.Y, valueMax.Z, valueMin.W] = this.ConvertToObject(item.range.max);
                    RangeValue.max = this.ArraytoVector(valueMax);
                }
    ...
    GenDefaultValue(typeName, funcName) {
        let OutRangeData = {};
        let [devData, userData] = this.ReadConfigData();
        if (devData == null)
            return;
